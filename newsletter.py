...
try:
    sql = "SELECT fName, email, Users.id, name FROM Users WHERE emailable=1 AND isReal=1"
    cursor.execute(sql)
...
for user in email_data:
    ...
    if (not strTo=='') and (unread_dripps > 0):
        ...
        fp = open("images/drop.svg", 'rb')
        msgImage = MIMEImage(fp.read())
        fp.close()
        msgImage.add_header('Content-ID', '<image4>')
        msgRoot.attach(msgImage)

        fp = open("images/bucketGrey.svg", 'rb')
        msgImage = MIMEImage(fp.read())
        fp.close()
        msgImage.add_header('Content-ID', '<image5>')
        msgRoot.attach(msgImage)

        fp = open("images/groupGrey.svg", 'rb')
        msgImage = MIMEImage(fp.read())
        fp.close()
